...
addParameter(p, 'mdlType2', {'bayes'}, @(x) iscell(x) && all(ismember(x, ["bayes", "sUAS"])));

defaultBayes = {
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200code_v2p1.txt'];
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200exclude_rotorcraft_v1p2.txt'];
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200only_rotorcraft_v1p2.txt'];
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200exclude_fwse_v1p2.txt'];
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200only_fwse_v1p2.txt'];
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200exclude_fwme_v1p2.txt'];
    [getenv('AEM_DIR_BAYES'), filesep, 'model', filesep, 'uncor_1200only_fwme_v1p2.txt']
};

addParameter(p, 'bayesFile1', defaultBayes, @(x) iscellstr(x));
addParameter(p, 'bayesFile2', defaultBayes, @(x) iscellstr(x));

addParameter(p,'iso_3166_2',{'US-MA'}) %;'US-HI';'US-KS';'US-MA';'US-MS';'US-NC';'US-ND';'US-NV';'US-NY'});
...
for m2Idx = 1:numel(parms.mdlType2)
    oldTradespace = tradespace;
    currentMdlType2 = parms.mdlType2{m2Idx};

    % Handle mdlType1 (assuming it's constant for this iteration)
    switch parms.mdlType1
        case 'bayes'
            bayes1 = parms.bayesFile1;
            geo1 = cellstr(char(32)); % Unicode whitespace
        case 'sUAS'
            geo1 = parms.iso_3166_2;
            bayes1 = string;
    end

    % Handle current mdlType2
    switch currentMdlType2
        case 'bayes'
            bayes2 = parms.bayesFile2;
            geo2 = cellstr(char(32)); % Unicode whitespace
        case 'sUAS'
    ...
    for ii=1:1:size(A,1)
        ...
        iiT.mdlType2 = repmat(currentMdlType2,nt,1);
    ...
    if height(allTradespaces) == 0
        allTradespaces = tradespace;
    else
        allTradespaces.mdlType2 = cellstr(allTradespaces.mdlType2);
        tradespace.mdlType2 = cellstr(tradespace.mdlType2);
        allTradespaces = [allTradespaces; tradespace];
    end
    
 end
 tradespace = allTradespaces; % replace tradespace with the final concatenated result
