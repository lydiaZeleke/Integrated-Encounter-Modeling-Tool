...
switch usecase
    case 'uncor_uncor'
        ...
        inFile = [getenv('INTEGRATED_ENC_DIR') filesep 'output' filesep sprintf('pairs-%s-anchorRange%0.2f.mat','uncor_uncor',anchorRange_nm)];
    ...
    otherwise
        ...
        for ii=uIso
            ...
            inFile = [getenv('INTEGRATED_ENC_DIR') filesep 'code' filesep sprintf('pairs-%s-anchorRange%0.2f.mat',sprintf('%s_%s',usecase,ii),anchorRange_nm)];
...
for ii=1:1:size(tradespace,1)
    ...
    switch usecase
        ...
        otherwise
            isIso = strcmpi(tradespace.subdivision2{ii},agg.iso_3166_2); 
    end
    nz = nnz(isIso);
